...
(defn generate-district-and-challenges [{:keys [district-hashes
  ...
  (let [accounts (web3-eth/accounts @web3)
        district (district-factory/approve-and-create-district
                   ...
                   {:from (first accounts)})
        _ (prn "DIST"district)
        district-tx-hash district
        _ (prn "distconst event"(contract-event-in-tx
                                  district-tx-hash
                                  :district-registry
                                  :DistrictConstructedEvent))
        reg-entry (-> district
...
;; TRY to replicate registry-entry-event-in-tx using DistrictConstructedEvent
    (prn "dnt" (dnt/balance-of (first accounts)))
    ;; (prn "reg entry" reg-entry "acct" (first accounts))
    ;; (prn "district token" (district/balance-of district (first accounts)))
