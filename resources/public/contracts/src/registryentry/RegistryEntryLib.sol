...
library RegistryEntryLib {
   ...
   */
  function isWinningOptionInclude(Challenge storage self)
    internal
    constant
    returns (bool) {
    return winningVoteOption(self) == VoteOption.Include;
  }
  ...
  function status(Challenge storage self)
    ...
    if (isChallengePeriodActive(self) && !wasChallenged(self)) {
      ...
      if (isWinningOptionInclude(self)) {
  ...
  function includeVotesAmount(Challenge storage self)
    private
    constant
    returns (uint) {
      return uint(self.stakeDelta + int(self.votesInclude) + int(registryToken.balanceOfAt(this, self.creationBlock)));
  }
  ...
  function winningVoteOption(Challenge storage self)
    ...
    uint votesInclude = includeVotesAmount(self);
    if (votesInclude.mul(100) > self.voteQuorum.mul(votesInclude.add(self.votesExclude))) {
  ...
  function winningVotesAmount(Challenge storage self)
    ...
    if (voteOption == VoteOption.Include) {
      return includeVotesAmount(self);
    } else if (voteOption == VoteOption.Exclude) {
      return self.votesExclude;
    } else {
      return 0;
    }
  }
